...
{
    [TestClass]
    public class StaleNoSerialize
    {
        [TestMethod]
        public void TestServeStaleNoSerialize()
        {
            // Set up the mocks
            ...
            cache.Verify(m => m.AddItem(testKey, It.IsAny<CacheElement>()), Times.AtLeast(2), "test item was added to the cache with no TTL");
            ...
            Assert.IsTrue(refreshserCalls >= 3, "cache was called at least three more times");
            cache.Verify(m => m.AddItem(testKey, It.IsAny<CacheElement>()), Times.AtLeast(3), "test item was added to the cache with no TTL");
